...
create table tb_score (value float8, user_id int8 not null, movie_id int8 not null, primary key (user_id, movie_id));
create table tb_user (id  bigserial not null, email varchar(255), primary key (id));
alter table if exists tb_score add constraint FKas3npxakhy1hwncegoa7hqn63 foreign key (user_id) references tb_movie;
alter table if exists tb_score add constraint FKrjjxs52ynheq6cy1qe4ej3p4p foreign key (movie_id) references tb_user;
